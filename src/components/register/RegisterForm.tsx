...
import { registerUser, updateDetails } from "@/apis/auth";
import {
  Button,
  Card,
  CardBody,
...
function RegisterForm({isEdit}:{isEdit?:boolean}) {
  ...
  useEffect(()=>{
    if(isEdit && authUser){
        const [countryCode, mobileNumber] = authUser.mobile.split("-");
        const [lCountryCode, lNumber] = authUser.landline.split("-");
        ...
        setValue("landlineCountryCode",lCountryCode)
        setValue("landlineNumber",lNumber)
    }
  },[isEdit,authUser])
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if ([400, 404].includes(error.status)) {
        toast.error(
          "Looks like the email is already used. Please contact your agency"
        );
      } else {
        ...
        <Card className={styles.card}>
          <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              }
            {
              ...
              >
                {loading ? <div className={styles.spinner}></div> : isEdit ? "Update Details" : "Register"}
              </Button>
            </CardBody>
          </Form>
        </Card>
